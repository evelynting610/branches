...
def init_clubs():
        global clubs
        global totals
        clubs.append(Club(list(), list(), [4,6,6,6], [10,0,8], dict(), list()))
        clubs.append(Club(list(), list(), [8,8,6,2], [12,0,12], dict(), list()))
        clubs.append(Club(list(), list(), [6,5,8,7], [12,0,8], dict(), list()))
        clubs.append(Club(list(), list(), [7,1,3,9], [10,0,10], dict(), list()))
        clubs.append(Club(list(), list(), [2,3,5,5], [9,0,6], dict(), list()))

        for entry in clubs:
                for i in range (0, 4):
                        totals.grades[i]+=entry.grades[i]
                for j in range(0, 3):
                        totals.genders[j]+=entry.genders[j]
...
def a_print_pretty():
        global clubs
        global email_to_clublist
        club_names = ["The 301 Branch", "The Ailurus Branch", "The Buena Vista Branch", "The Fourth Wall", "The SOS Branch"]
        ...
        for ix in range(NUM_CLUBS):
                ...
                first_choice = str(num_first_choice)+" out of "+str(num_members)+" new members marked this as their first choice\n"
